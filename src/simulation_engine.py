...
class MarginEngine:
    ...
    def calculate_stock_portfolio_value(self, current_prices: Dict[str, float]) -> float:
        ...
        for ticker, shares in self.state.holdings.items():
            total += shares * current_prices.get(ticker, 0.0)
        # Pending delivery shares (exposed to price action but cannot be sold yet)
    ...
    def execute_order(
        ...
        if action == "BUY":
            if not use_margin:
                ...
                required_cash = order_cost * self.initial_margin_ratio
                if self.state.cash < required_cash:
                    return False

                borrow_amount = max(0.0, order_cost - self.state.cash)
                self.state.cash = max(0.0, self.state.cash - order_cost)
                self.state.margin_debt += borrow_amount

            # Settlement Delay Mechanism (from Phase 1 Workflow):
            # If tick < 150: Deliver after T+0.5 (tick + 30)
            # If tick >= 150: Deliver in Phase 2
            if current_tick < 150:
                delivery_tick = current_tick + 30
                target_phase = current_phase
            else:
                delivery_tick = 301  # Starts Phase 2
                target_phase = current_phase + 1

            self.state.pending_shares[ticker] = self.state.pending_shares.get(ticker, 0.0) + shares
            self.state.pending_queue.append(
                PendingDelivery(
                    ticker=ticker,
                    shares=shares,
                    delivery_tick=delivery_tick,
                    deliver_in_phase=target_phase,
                )
            )
            return True
